# Jaunt Data Scout – MVP Epics and Tasks (v2, Google in scope)

This document enumerates the MVP Epics and their Tasks derived from the v2 System Design (Google in scope). Use it to drive issue creation and tracking.

How to use
1) Labels: create `MVP`, `backend`, `data`, `API`, `infra`, `LLM`, `Postgres`, `observability`, `ETL`, `QA`, `google`, `compliance`, `fastapi`.
...
Goals
- Implement AWS Step Functions orchestration for the MVP pipeline: DiscoverTargets → SeedPrimaries → ExpandNeighbors → TileSweep → ValidateDedupe → Persist → Rank → Finalize.
- Integrate SQS frontier + DLQ, S3 raw cache, budgets, early-stop, and metrics/traces.

Acceptance criteria
- A city job completes end-to-end for Edinburgh using mixed connectors (Google + open).
- State transitions logged; failures to DLQ; raw responses cached to S3 by (source, request_hash).
- Budget controls enforced (per-connector caps & 70/30 primaries/secondaries); early-stop when new_unique_rate < 5% over last 200 calls; wall-clock ≤ 6 hours respected.
- CloudWatch metrics and OTEL traces emitted per state; kill switches supported via job params.

Tasks
- [ ] Define Step Functions state machine with all v2 states, retries/backoff, budget tokens, and early-stop gates.
- [ ] Budget manager: token buckets per connector (google.text, google.nearby, google.details, overpass, otm, wiki, tavily, nominatim) with 70/30 allocation controls.
- [ ] Implement frontier SQS schemas (seed/expand/tile_sweep/open_data_pull) and DLQ; include budget_token and city context.
- [ ] S3 layout and lifecycle: raw/{source}/{city}/{hash}.json, manifests/{city}/targets-v1.json; lifecycle for Google 30d, open-data 90d.
- [ ] Orchestrator config: YAML defaults for budgets, concurrency, early-stop; run-level overrides.
- [ ] Emit metrics/alerts per state; add tracing context propagation across SQS and Step Functions.
- [ ] Kill switches and circuit breakers per connector; job resume/reentry semantics.
...
- [ ] Implement Details with explicit field masks: must-haves (place_id, name, formatted_address, geometry/location, types, rating, user_ratings_total, business_status, photos.photo_reference, html_attributions); optional nice-to-haves gated by budget (opening_hours, website, international_phone_number, editorial_summary).
...
## Epic: Tavily + LLM City Data Targets Manifest (v2)
Labels: MVP, LLM, data

Goals
- Generate a manifest of open-data portals, datasets, and endpoints per city using Tavily + Gemini prompts T1–T4.
...
Tasks
- [ ] Primary seeding from Google Text + OTM + Wikidata/Wikipedia + open-data endpoints; pre-rank primaries; schedule Google Details calls for primaries only.
...
## Epic: Extraction & Normalization — Structured JSON via readers + Gemini T5 fallback
Labels: MVP, LLM, ETL

Goals
- Prefer machine-readable ingestion; fall back to Gemini extractor for HTML when APIs absent; ensure country-aware normalization.
...
- [ ] Gemini T5 extractor with bounded JSON schema, timeouts, and token/cost guards.
- [ ] Address normalizer; Nominatim client (≤1 QPS, ≤1k calls) as fallback only.
- [ ] Field mappers to canonical candidate schema; capture google/open-data specific fields in additional_content sub-objects.
...
- [ ] Merge strategy, conflict resolution with Gemini tie-break for top conflicts; reversible lineage.
...
Tasks
- [ ] Implement primary composite: w_pop*(Google rating + log(reviews+1), OTM rate) + w_auth*(Wikidata/Wikipedia/registries) + w_geo*(geo_centrality) + w_nov*(niche/open-data) + w_net*(graph_context).
...
- [ ] S3 lifecycle policies: Google raw 30d, open-data 90d; encryption SSE-S3/KMS.
...
Acceptance criteria
- CloudWatch dashboards and alarms per state and connector; OTEL traces stitched across workers.
- Cost estimation metrics and budget breach alerts; new_unique_rate tracked.

Tasks
- [ ] Define metrics schema: calls, errors, backoffs, new_unique_rate, dedupe ratio, H3 coverage, cost estimate, duration.
- [ ] Tracing propagation (OTEL) across orchestrator, SQS, connectors, LLM, DB writes.
- [ ] Alarm definitions and runbooks: error spikes, low new_unique_rate, nearing budget caps, DB write failures, under-coverage.

## Epic: Security & Compliance — Google & Data handling
Labels: MVP, infra, compliance

Goals
- Enforce secrets management, IAM least privilege, encryption, and Google Maps Platform compliance.
...
- Partner API default filtering excludes Google-only fields; audit logging for Details calls and field masks.

Tasks
- [ ] Secrets Manager wiring (Google, Tavily, OTM, DB); rotation policy.
- [ ] IAM roles and S3 bucket policies; SSE-S3/KMS; access logs enabled.
- [ ] Compliance middleware/guards: enforce field masks, response filtering, retention tagging, attribution presence.
- [ ] Audit logs for Details calls (fields requested, purpose, timestamp).
...
- Compliance: Google raw retention ≤30 days verified; API responses filtered by default; attribution present where required.
...
- [ ] Budget caps per connector and etiquette/compliance — Labels: MVP, infra, backend, compliance
...
- [ ] Compliance guardrails: field masks, retention, attribution — Labels: MVP, compliance, google
